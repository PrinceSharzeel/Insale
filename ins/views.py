...
def index(request):
	...
	if request.method=='POST':
		...
		if form.is_valid():
			...
			details={'name':name,'mobile':mobile,'email':email,'location':location,'pica':picture_a,'picb':picture_b,'picc':picture_c, 'time':timestamp,'room':room,'title':title,'desc':desc,'cat':cat,'price':price,'date':date,'verified':'no'}
...
def prod_details(request,prod_name):
	title=prod_name
	a=user_db.ads.find_one({'title':title})
	details={'name':a['name'],'mobile':a['mobile'],'email':a['email'],'pica':a['pica'],'picc':a['picc'],'picb':a['picb'],'date':a['date'],'time':a['time'],'location':a['location'],'room':a['room'],'title':a['title'],'desc':a['desc'],'cat':a['cat'],'price':a['price']}
...
def owner(request):
	...
	print(revenue)
	return render(request,'dashboard.html',{'prods':ls,'revenue':revenue,'counter':counter,'total':tot})
...
def product_update(request,prodname):
	...
	if request.method=='POST':
		...
		if form.is_valid():
			...
			price=form.cleaned_data['fprice']
			date=form.cleaned_data['fdate']
			details={'name':name,'mobile':mobile,'email':email,'location':location,'room':room,'title':title,'desc':desc,'cat':cat,'price':price,'date':date}
			...
			user_db.ads.update_one(
				        {"title": prodname},
				        {
				        "$set": {'name':name,'mobile':mobile,'email':email,'location':location,'room':room,'title':title,'desc':desc,'cat':cat,'price':price,'date':date
	...
	a=user_db.ads.find_one({'title':prodname})
	print(a)
	details={'name':a['name'],'mobile':a['mobile'],'email':a['email'],'location':a['location'],'pica':a['pica'],'picc':a['picc'],'picb':a['picb'],'pica':a['pica'],'picc':a['picc'],'picb':a['picb'],'room':a['room'],'title':a['title'],'desc':a['desc'],'cat':a['cat'],'price':a['price'],'date':a['date']}
	print(details)
...
def rempic(request,pic):
	...
	if a is not None:
		user_db.ads.update_one(
				        {"pica":pic},
				        {
				        "$set": {'pica':'/static/media/default.jpg'
	...
	if b is not None:
		user_db.ads.update_one(
				        {"picb":pic},
				        {
				        "$set": {'picb':'/static/media/default.jpg'
	...
	if c is not None:
		user_db.ads.update_one(
				        {"picc":pic},
				        {
				        "$set": {'picc':'/static/media/default.jpg'
...
def signup(request):
    if request.method=='GET':
        try:
            ...
            if True:
                authorization_base_url = 'https://www.facebook.com/dialog/oauth/?scope=email,public_profile'
                token_url = 'https://graph.facebook.com/oauth/access_token'
                ...
                try:
                    ...
                    try:
                        ...
                        f=open(log_file,"a")
                        print(str(getframeinfo(currentframe()).lineno)+" - "+str(e),file=f)
                        f.close()
                        raise
                    u = get_object_or_404(users, uid=entry["id"])
                    flag = False
                    login(request, user)
                    u.picture = "https://graph.facebook.com/"+u.uid+"/picture?type=large"
                    u.save()
                    ...
                    entry1 = users(
                                ...
                                email=entry["email"],
                                uid = entry["id"],
                                picture = "https://graph.facebook.com/"+entry["id"]+"/picture?type=large"
                            )
...
def stock(request):
	...
	return render(request,'stock.html',{'ls':ls,'mo':mo,'fa':fa,'ex':ex,'bsh':bsh,'ea':ea,'fur':fur})
	...
	if request.method=='POST':
		...
		if form.is_valid():
			...
			if len(feedback)!=0:
				print(feedback)
				try:
					user_db.feed.insert({'name':name,'mail':email,'lname':lname,'msg':feedback})
				except:
...
def namecont(request):
	if request.method=='POST':
		...
		if form.is_valid():
			...
			ls=user_db.ads.find({'verified':'yes','title':{'$regex': key, '$options': 'i'}})
			if ls is None:
